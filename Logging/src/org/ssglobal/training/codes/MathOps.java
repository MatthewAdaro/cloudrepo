...
public class MathOps {
	...
	public int add(int x, int y) {
		logger.info("start of method add() with parameter x=%d, y=%d".formatted(x, y));

		logger.warning("using custom Date() object");
		Date xmas = new Date(123, 1, 32);
		System.out.println(xmas);
		int sum = x + y;
		logger.info("end of method add() with parameter x=%d, y=%d".formatted(x, y));
		return sum;
	}

	public int div(int x, int y) throws ArithmeticException, Exception {
		logger.info("start of method div() with parameter x=%d, y=%d".formatted(x, y));
		if (y == 0) {
			logger.severe("division by zero error");
			throw new ArithmeticException();
		}

		logger.info("end of method div() with parameter x=%d, y=%d".formatted(x, y));
		return x / y;
	}
